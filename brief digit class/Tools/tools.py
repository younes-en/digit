...
def collection():
    ...
    name_generator = chr(np.random.randint(500)) + \
        chr(np.random.randint(500))+chr(np.random.randint(500))
    ...
    for ii in range(0, 10):
        ...
        if any(File.endswith(".csv") for File in os.listdir('./Dataset/')):
            ...
            df.to_csv('./DataSet/DataSet__' +
                      name_generator+'__.csv', index=False)
    return df.head()
